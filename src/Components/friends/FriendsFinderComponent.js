...
class FriendsFinderComponent extends AuthGuardedComponent {
  ...
  updateRedux = () => {
    this.customAxios
      .get(`${process.env.REACT_APP_API_HOST}sersNotFriend`)
      .then(res => {
  ...
  sendFriendshipRequest = user => {
    this.customAxios
      .get(`${process.env.REACT_APP_API_HOST}addFriend/${user.id}`)
      .then(res => {
        if (res.data.message) {
          this.props.dispatch(
            notify({
  ...
  removeFriendhip = user => {
    this.customAxios
      .get(`${process.env.REACT_APP_API_HOST}/removeFriendship/${user.id}`)
      .then(res => {
        if (res.data.message) {
          this.props.dispatch(
            notify({
